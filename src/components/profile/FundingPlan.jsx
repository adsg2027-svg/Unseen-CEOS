...
export default function FundingPlan({ entrepreneur }) {
  const timelineSteps = [
    { labelKey: 'Short-term',  desc: entrepreneur.growthPlan?.shortTerm,  color: 'bg-primary-500' },
    { labelKey: 'Medium-term', desc: entrepreneur.growthPlan?.mediumTerm, color: 'bg-amber-500'   },
    { labelKey: 'Long-term',   desc: entrepreneur.growthPlan?.longTerm,   color: 'bg-green-500'   },
  ].filter(s => s.desc);

  return (
    <div className="space-y-6">
      <Card title={<T>Funding Requirements</T>} icon={Wallet}>
        <div className="flex flex-col sm:flex-row items-start gap-6">
          <div className="bg-primary-50 border border-primary-200 rounded-xl p-6 text-center sm:w-48 shrink-0">
            <p className="text-xs text-primary-600 font-medium mb-1"><T>Funding Needed</T></p>
            <p className="text-3xl font-bold text-primary-700">{formatINR(entrepreneur.fundingNeeded)}</p>
          </div>
          <div className="flex-1">
            <div className="mb-4">
              <p className="text-sm font-medium text-warm-700 mb-1"><T>Purpose</T></p>
              <p className="text-sm text-warm-600 leading-relaxed">{entrepreneur.fundingPurpose}</p>
            </div>
            <div>
              <p className="text-sm font-medium text-warm-700 mb-2"><T>Current Funding Sources</T></p>
      ...
      {timelineSteps.length > 0 && (
        <Card title={<T>Growth Plan</T>} icon={TrendingUp}>
            ...
            <div className="space-y-6">
              {timelineSteps.map((step, i) => (
                <div key={i} className="relative">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-warm-800"><T>{step.labelKey}</T></p>
      ...
      {entrepreneur.interviewNotes && (
        <Card title={<T>Interview Notes</T>} icon={Target}>
          <p className="text-sm text-warm-600 leading-relaxed">{entrepreneur.interviewNotes}</p>
          <div className="mt-4 flex flex-wrap gap-2">
            <span className="text-xs text-warm-400"><T>Interviewed by:</T> {entrepreneur.interviewedBy}</span>
            <span className="text-xs text-warm-400">|</span>
            <span className="text-xs text-warm-400"><T>Date:</T> {entrepreneur.interviewDate}</span>
          </div>
          {entrepreneur.challenges?.length > 0 && (
            <div className="mt-4">
              <p className="text-sm font-medium text-warm-700 mb-2"><T>Key Challenges</T></p>
              <div className="flex flex-wrap gap-2">
